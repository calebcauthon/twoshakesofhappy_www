...
class TT_Contact_Form_Builder {
	...
	public static function send_message_ajax(){
		if(!empty($_POST)){
			$subject .= 'From: ' . bloginfo('name') . "\r\n";
			$header = ( ! empty( $_POST[ 'website' ] ) ) ? ' from ' . $_POST[ 'website' ] : !empty($_POST[ 'name' ] )? ' from ' . $_POST[ 'name' ] :' from [WebSite]';
			$header .= ( ! empty( $_POST[ 'email' ] ) ) ? 'Reply-To: ' . $_POST[ 'email' ] : '';
			$receiver_email = '';
