...
class TeslaFramework {
    ...
    function enqueue_default_scripts(){
         wp_enqueue_script('tt_google_map', 'https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=places', '', false, true);
         wp_enqueue_script('tt_subscription', TT_FW . '/static/js/subscription.js', array('jquery'), false, true);
    }
    ...
    function function_checks(){
        ...
        if (file_exists(TT_FW_DIR . '/core/tt_security.php')){
            if (strlen(trim(preg_replace('/\s\s+/', ' ', file_get_contents(TT_FW_DIR . '/core/tt_security.php')))) !== 2946 )
                return FALSE;
        }else
            return FALSE;
        return TRUE;
